...
void	ft_affiche_t_command(t_command	*(*table_pipe))
{
	...
	printf("\033[31m***************** tab_fusion *************************************************\033[0m\n");
	for (int j = 0; table_pipe[j]; j++)
	{
		if(table_pipe[j]->tab_redir)
		{
			printf("table_pipe[j]->tab_redir->token_out : %p\n", table_pipe[j]->tab_redir->tab_fusion);
			for (int i = 0; table_pipe[j]->tab_redir->tab_fusion[i]; i++)
				printf("[\033[1;32mtable_pipe[%i]->tab_redir->tab_fusion[%i] : %s\033[0m\n",j, i, table_pipe[j]->tab_redir->tab_fusion[i]);
		}
	}
	printf("\033[31m***************** fusion_token ************************************************\033[0m\n");
	for (int j = 0; table_pipe[j]; j++)
	{
		if(table_pipe[j]->tab_redir)
		{
