...
/*   Updated: 2022/05/31 03:00:16 by aptive           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_affiche_t_command(t_command	*(*table_pipe))
{
	...
	for (int j = 0; table_pipe[j]; j++)
	{
		if(table_pipe[j]->tab_redirection)
		{
			printf("table_pipe[j]->tab_redirection->token_in : %p\n", table_pipe[j]->tab_redirection->token_in);
			for (int i = 0; table_pipe[j]->tab_redirection->token_in[i]; i++)
				printf("[\033[1;32mtable_pipe[%i]->tab_redirection->token_in[%i] : %s\033[0m\n",j, i, table_pipe[j]->tab_redirection->token_in[i]);
		}
	}
	...
	for (int j = 0; table_pipe[j]; j++)
	{
		if(table_pipe[j]->tab_redirection)
		{
			printf("table_pipe[j]->tab_redirection->token_out : %p\n", table_pipe[j]->tab_redirection->token_out);
			for (int i = 0; table_pipe[j]->tab_redirection->token_out[i]; i++)
				printf("[\033[1;32mtable_pipe[%i]->tab_redirection->token_out[%i] : %s\033[0m\n",j, i, table_pipe[j]->tab_redirection->token_out[i]);
		}
	}
